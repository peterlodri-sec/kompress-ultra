 ...
 *
 * Default: local hash embedding with no external dependencies.
 *
 * OVHCloud:
 * set KOMPRESS_USE_CLOUD_EMBEDDING=1 +
 * OVHCLOUD_API_KEY +
...
export async function embedText(
  ...
  // Cloud path — opt-in via environment flag.
...
/**
 * OVHCloud embedding — kept as opt-in fallback.
 * Not used by default.
 */
async function embedTextCloud(
  text: string,
): Promise<number[] | null> {
  const endpoint =
...
/**
 * Score a message's relevance using embeddings.
 *
 * Compares message content against a task goal via cosine similarity.
 *
 * `goalEmbedding` may be supplied when many messages are being compared
 * against the same task goal. This avoids repeatedly hashing the identical
 * goal for every message.
 *
 * Existing callers can continue to pass only `text` and `sliceId`.
 *
 * @param text — message content to score
 * @param sliceId — task goal / context to compare against
 * @param goalEmbedding — optional precomputed embedding of the task goal
 */
export async function scoreMessageLocal(
  ...
  try {
    ...
    //
    // If the caller has already embedded the task goal, reuse that vector.
    // Otherwise preserve the original behavior and compute it here.
    if (goalEmbedding) {
    ...
    // Silent — embedding failure returns 0.5 neutral score.
    return 0.5;
  }
}
 ...
 *
 * Returns maximum cosine similarity score from stored entries.
 */
 ...
 *
 * Returns formatted strings such as:
 * "[pattern_type] summary (conf=0.95)"
 */
...
/**
 * Write dropped message content to local vector store and persist.
 *
 * Stores each message as a vector for future similarity search.
 *
 * Note: IDs retain the existing timestamp/random scheme here so this
 * optimization does not alter memory/deduplication semantics.
 */
